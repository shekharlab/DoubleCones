...
# rotated_images <- lapply(image.files, rotate_image, angle = -90)
# lapply(rotated_images, function(x) )
```
...
# Goldfish from Kon et al. 2022

```{r}
goldfish.conversion = data.frame(old.names = c("L_cone", "M_cone", "Rod"), 
                                 new.names = c("red", "green", "rod"))

objectList$Goldfish$annotated = goldfish.conversion$new.names[match(objectList$Goldfish$lit_type, goldfish.conversion$old.names)]
```

```{r, fig.height=5, fig.width=15}
AnnotatedUmap(objectList$Goldfish, pr_annotation, color.clusters.by = "annotated", group.by = "annotated", rel_widths = c(1,1.5), color.genes = TRUE)
```

Trying to salvage blue cones fails

```{r, fig.height=10, fig.width=10, eval = FALSE}
goldfish.split = SplitObject(objectList$Goldfish, split.by = "animal")
goldfish.split = lapply(goldfish.split, ClusterSeurat)

# Blue cones do not subcluster in any of the biological replicates; its not a batch integration error
plot_grid(plotlist = lapply(goldfish.split, function(object) FeaturePlot(object, features = "opn1sw2")), labels = names(goldfish.split))
plot_grid(plotlist = lapply(goldfish.split, function(object) FeaturePlot(object, features = "opn1lw1")), labels = names(goldfish.split))
plot_grid(plotlist = lapply(goldfish.split, function(object) FeaturePlot(object, features = "opn1mw4")), labels = names(goldfish.split))
```

```{r, fig.height=4, fig.width=12, eval = FALSE}
best = subset(goldfish.split$`Wakin-2`, annotated %in% c("red", "green"))
best = ClusterSeurat(best)
FeaturePlot(best, features = c("opn1sw2", "opn1lw1", "opn1mw4"), ncol = 3)
```

Does opn1sw2 have mutually exclusive expression with other cone opsins? 

```{r, eval = FALSE}
FeatureScatter(subset(objectList$Goldfish, annotated %in% c("red", "green")), feature1 = "opn1sw2", feature2 = "opn1mw4")
FeatureScatter(subset(objectList$Goldfish, annotated %in% c("red", "green")), feature1 = "opn1sw2", feature2 = "opn1lw1")
FeatureScatter(subset(objectList$Goldfish, annotated %in% c("red", "green")), feature1 = "opn1mw4", feature2 = "opn1lw1")
# FeatureScatter(objectList$Goldfish, feature1 = "opn1mw4", feature2 = "arid2")

FeaturePlot(objectList$Goldfish, features = "arid2")
```

```{r, fig.height=5, fig.width=5, eval = FALSE}
Idents(objectList$Goldfish) = "annotated"

ExpressionScatter(objectList$Goldfish, x = "red", y="green", logX = FALSE, logY = FALSE, genes.to.label = c("opn1sw2", "opn1mw4", "opn1lw1", "rho"))
ExpressionScatter(objectList$Goldfish, x = "red", y="rod", logX = FALSE, logY = FALSE, genes.to.label = c("opn1sw2", "opn1mw4", "opn1lw1", "rho"))
ExpressionScatter(objectList$Goldfish, x = "rod", y="green", logX = FALSE, logY = FALSE, genes.to.label = c("opn1sw2", "opn1mw4", "opn1lw1", "rho"))
```
...
# objectList$Chicken = MapBack(objectList$Chicken, chick.pr, group.by = "NAME", convert.barcodes = TRUE)
# objectList$Chicken$id = str_split_fixed(objectList$Chicken$mapped.back, '_', 2)[,1]
```
...
Reproducing results from Josh/Ayush
...
# objectList$Opossum = ReprocessIntegrated(objectList$Opossum)
# DimPlot(objectList$Opossum, group.by = "annotated")
...
Can find rods easily enough, but too few cones

```{r, eval = FALSE}
# object = readRDS("../../Species_Objects/Cone_Objects/Zebrafish_PR_int_v2.rds")
full = readRDS("../../Species_Objects/Zebrafish_initial.rds")
full = UpdateEnrichment(full, "Zebrafish")
DimPlot(full)
DotPlot2(full, features = unique(toupper(Genes(major_annotation))))
DotPlot2(full, features = unique(toupper(Genes(pr_annotation))))
```

```{r, eval = FALSE}
object = subset(full, seurat_clusters %in% c(12, 18, 27))
object = Harmonize(object, batch = "animal")
DimPlot(object, group.by = "animal")
FeaturePlot(object, "OPN1SW2", order = TRUE)
FeaturePlot(object, "OPN1SW1", order = TRUE)
FeaturePlot(object, "OPN1LW1", order = TRUE)
FeaturePlot(object, "OPN1LW2", order = TRUE)
DotPlot2(object, features = unique(toupper(Genes(pr_annotation))))
DotPlot2(object, features = unique(toupper(Genes(major_annotation))))

ze_pr = subset(object, seurat_clusters %in% c(6))
DimPlot(ze_pr)
```
...
```{r, fig.height=5, fig.width=15}
# object = readRDS("../../Species_Objects/Cone_Objects/Rat_PR_v2.rds")
DefaultAssay(objectList$Rat) = "RNA"
objectList$Rat = UpdateEnrichment(objectList$Rat, "Rat")
plotBatches2(objectList$Rat, 'animal')
objectList$Rat = Harmonize(objectList$Rat, batch = "animal")
objectList$Rat$annotated = AnnotateClusters(objectList$Rat, data.frame(old.names = c(0,1,2,3,4,5,6,7), 
                                                                       new.names = c("rod", "rod", "rod", "rod","red", "rod", "rod", "UV")))
AnnotatedUmap(objectList$Rat, pr_annotation, color.clusters.by = "annotated", group.by = "annotated", rel_widths = c(1,1.5), color.genes = TRUE)
```

Rat has the same shrapnel as opossum

# Mouse

```{r, fig.height=5, fig.width=15}
mouse.conversion = data.frame(old.names = c("Rod", "mlCone", "sCone"),
                              new.names = c("rod", "red", "UV"))

mouse$annotated = mouse.conversion$new.names[match(mouse$type, mouse.conversion$old.names)]

AnnotatedUmap(mouse, pr_annotation, color.clusters.by = "annotated", group.by = "annotated", rel_widths = c(1,1.5), color.genes = TRUE)
```
...
## Lyu adult
# ze_adult = readRDS("../../Species_Objects/Cone_Objects/Lyu_Zebrafish_PR_adult.rds") # use version 1 since 2 was renamed to be compatible with Zebrafish_Key but have since made Zebrafish_Lyu_key
```
...
# objectList = mclapply(metadata$speciesList, function(species) readRDS(paste0("../../Species_Objects/Cone_Objects/", species, "_clean_v1.rds")), mc.cores = 1)
# names(objectList) = metadata$speciesList

# For the ENSEMBL-aligned zebrafish data
# objectList$Zebrafish = readRDS("../../Species_Objects/Cone_Objects/Zebrafish_clean_v2.rds")

# For chick with corrected opsin
# objectList$Chicken = readRDS("../../Species_Objects/Cone_Objects/Chicken_clean_v2.rds")
# objectList$Chicken = readRDS("../../Species_Objects/Cone_Objects/Chicken_clean_v3.rds")

# Lyu developing
# objectList$ZebrafishDev = readRDS("../../Species_Objects/Cone_Objects/Lyu_Zebrafish_PR2.rds")

# Lyu adult
# objectList$ZebrafishAdult = readRDS("../../Species_Objects/Cone_Objects/Lyu_Zebrafish_PR_adult2.rds")

# Modify name
# levels(objectList$ZebrafishCorbo$annotated) = c(levels(objectList$Zebrafish$annotated), "opn1mw4/\nopn1lw1")
# objectList$Zebrafish$annotated[objectList$Zebrafish$annotated == "opn1mw4/\nopn1lw1"] = "opn1mw4/opn1lw1"
# objectList$Zebrafish$annotated[objectList$Zebrafish$annotated == "opn1mw4/opn1lw1"] = "opn1mw4/\nopn1lw1"
...
# Remove goldfish 
# objectList$Goldfish = NULL

# Add animal id to all species
# objectList$Zebrafish$animal = "S1"
...
# DCb is actually principle + accessory in same droplet!

Is chicken DBCones3 really a full of DBCone1/2? 
...
```{r}
results = lapply(seq(0, 1, by = 0.01), function(alpha) {
  lapply(seq(0, 1, by = 0.01), function(beta){
    r.squared = cor(avgexpr$DCb, alpha*avgexpr$DCa + beta*avgexpr$DCc)^2
    rmse = RMSE(avgexpr$DCb, alpha*avgexpr$DCa + beta*avgexpr$DCc)
    return(data.frame(alpha = alpha, beta = beta, r.squared = r.squared, rmse = rmse))
  })
})

param_summary = do.call(rbind, unlist(results, recursive = FALSE))
param_summary$color[which.min(param_summary$rmse),] = "red"

param_summary$color = 'yellow'
param_summary$color[param_summary$alpha > 0.5 & param_summary$beta > 0.5] = 'lightblue'
param_summary$color[param_summary$alpha <= 0.5 & param_summary$beta > 0.5] = 'lightgreen'
param_summary$color[param_summary$alpha <= 0.5 & param_summary$beta <= 0.5] = 'violet'

param_summary$color[param_summary$alpha > 0.45 & param_summary$beta > 0.45 &
                      param_summary$alpha < 0.55 & param_summary$beta < 0.55] = 'red'

ggplot(na.omit(param_summary), aes(alpha, beta, z=rmse, color=color)) +
  axes_3D() +
  # stat_wireframe(alpha=.5, theta=150) +
  stat_3D(aes(color=rmse), alpha=.5) +
  theme_void() +
...
# Cell type proportions across species

For in situ, keep DC-A and DC-P separate
...
# chicken.table = chicken.table/rowSums(chicken.table) # Renormalize
chicken.table
chicken.table/rowSums(chicken.table)
```
...
```{r pr_summary, fig.height=25, fig.width=10}
plot_grid(plotlist = lapply(seq_along(objectList), function(index) AnnotatedUmap(objectList[[index]], 
                                                                                 pr_annotation, color.clusters.by = "annotated", group.by = "annotated", 
                                                                                 rel_widths = c(1,1.7), color.genes = FALSE, title = names(objectList)[[index]], 
                                                                                 coord_flip = FALSE)), 
          nrow = length(objectList), align = 'v', axis = 'lr')
```
...
# objectList$ChickenV2 = subset(objectList$ChickenV2, annotated != "full")

names(objectList) = c("Zebrafish", "Chicken", "Lizard", "Opossum", "Squirrel", "Human")

# Rename opossum accessory cluster as unknown
# objectList$Opossum$annotated[objectList$Opossum$annotated == "accessory"]
```
...
p1 = plot_grid(
  ...
  # ggarrange(plotlist = lapply(FeaturePlot(subset(fullList$ZebrafishAdult, annotated %in% c('red', 'green', 'opn1mw4/\nopn1lw1')), c('opn1lw1', 'opn1lw2', 'opn1mw1', 'opn1mw2', 'opn1mw3', 'opn1mw4'), reduction = 'umap', combine = FALSE), function(plot) theme_umap(plot + scale_color_gradient(limits = c(0,6), low = 'grey', high = 'blue', na.value = 'blue'), remove.axes = FALSE)), 
  #                           common.legend = TRUE, nrow = 6, legend = 'right'),
...
# feature.plots = lapply(FeaturePlot(subset(fullList$ZebrafishAdult, annotated %in% c('red', 'green', 'opn1mw4/\nopn1lw1')), c('opn1lw1', 'opn1lw2', 'opn1mw1', 'opn1mw2', 'opn1mw3', 'opn1mw4'), reduction = 'umap', combine = FALSE), function(plot) theme_umap(plot + scale_color_gradient(limits = c(0,6), low = 'grey', high = 'blue', na.value = 'blue'), remove.axes = FALSE))
# 
# plotlist = list(feature.plots[[1]], 
#                 feature.plots[[2]], 
#                 NULL, 
#                 NULL, 
#                 feature.plots[[3]], 
...
#                 )
# 
# p3 = ggarrange(plotlist = plotlist, 
#                common.legend = TRUE, ncol = 4, nrow = 2, legend = 'right', labels = c('C'), font.label = list(size = 20))
```

Try using PCA instead of UMAP

```{r}
# ze_subset = subset(fullList$ZebrafishAdult, annotated %in% c('red', 'green', 'opn1mw4/\nopn1lw1'))
# ze_subset = ClusterSeurat(ze_subset)
# DimPlot(ze_subset, group.by = 'annotated')
# DimPlot(ze_subset, group.by = 'annotated', reduction = 'pca')
...
# plotlist = append(plotlist, legend)

# p2 = plot_grid(theme_umap(DimPlot(ze_subset, group.by = 'annotated', reduction = 'pca', cols = pr_palette) + NoLegend(), title = '') + xlab('PC1') + ylab('PC2'), NULL, NULL, NULL, nrow = 2, ncol = 2)
...
# plot_grid(p1, p3, rel_widths = c(2,1), nrow = 2)
...
# Rotate rods to top
# angles = c(pi/2, 4*pi/5, pi/12, -pi/3, -3*pi/5, 3*pi/5)
...
```{r pr-summary-prop, fig.height=5, fig.width=7}
# Rename lizard opsins
# objectList$Lizard = fullList$Lizard
# objectList$Squirrel = fullList$Squirrel

# objectList$Lizard = RenameFeatures(fullList$Lizard, new.names = gsub('LOC132767849', 'OPN1LW', gsub('LOC132773706', 'OPN1MW', gsub('LOC132767847', 'OPN2SW', rownames(fullList$Lizard)))))

# Rename squirrel red opsin
# objectList$Squirrel = RenameFeatures(fullList$Squirrel, new.names = gsub('ENSSTOG00000024701', 'OPN1LW', rownames(fullList$Squirrel)))

# DotPlot2(objectList$Lizard, features = c('OPN1LW', 'OPN1MW', 'OPN2SW', 'LOC132773706'), group.by = 'annotated', assay = 'RNA')
```

Zebrafish ortholog of calbindin

```{r}
ze2ch = readRDS('orthology_graphs/ze2ch.rds')
ConvertGeneSymbolsFromMatrix(t(ze2ch), c('OPN1LW', 'CALB1', 'OPN1MSW', 'SAMD7'))
```


```{r pr-summary-prop, fig.height=15, fig.width=3}
dotplot.list = lapply(seq_along(objectList), function(index) {
  AnnotatedDotPlot(objectList[[index]], pr_annotation2, group.by = "annotated", color.clusters.by = "annotated", assay = 'RNA', features = if(index == 1) setdiff(rev(Genes(pr_annotation2)), c("zeb2b", "RHO")) else rev(Genes(pr_annotation2)), color.genes = FALSE, coord.flip = FALSE) + 
    NoLegend() + 
    RotatedAxis() + 
...
dotplots = plot_grid(plotlist = Interleave(dotplot.list),
                       #append(dotplot.list[1:3], list(NULL, dotplot.list[[5]], NULL, dotplot.list[[5]], NULL, dotplot.list[[6]])), 
                     nrow = length(Interleave(dotplot.list)), 
  align = 'v', 
  axis = 'lr', 
  rel_heights = c(0.9, 0.01,
                  1.1, -0.01,
                  1.1, 0.01, 
                  0.80, 0.12, 
                  0.65, 0.24, 
...
dotplots + theme(plot.margin = unit(c(0,0,0,0.5), 'cm'))

ggsave('figures/fig1_dotplots.pdf', height=15, width=2.9)
```

```{r, fig.height=15, fig.width=4}
dotplot.list.legend = lapply(seq_along(objectList), function(index) {
  AnnotatedDotPlot(objectList[[index]], pr_annotation2, group.by = "annotated", color.clusters.by = "annotated",
                       features = if(index == 1) setdiff(Genes(pr_annotation2), c("zeb2b", "RHO")) else Genes(pr_annotation2), color.genes = FALSE) + RotatedAxis() + theme(axis.title = element_blank(), axis.text.y = element_blank())
  })
...
# Cell type markers within species

```{r, fig.height=5, fig.width=15}
markersList = lapply(objectList, function(object) TopNDEGs(DownsampleSeurat(object, group.by = "annotated", size = 100), group.by = "annotated", n = 40))
```

```{r pr-heatmaps, fig.height=30, fig.width=10}
plot_grid(
  plotlist = lapply(seq_along(objectList), function(index) DoHeatmap(DownsampleSeurat(objectList[[index]], group.by = "annotated", size = 50), features = markersList[[index]]$feature, group.by = "annotated") + NoLegend() + scale_fill_gradientn(colors = c("blue", "white", "red")) + theme(axis.text.y = element_blank())),
  nrow = length(objectList), 
  labels = names(objectList),
  hjust = 0,
  align = "hv", 
  axis = "bt"
)
...
```{r}
coefficients.list = lapply(c("Chicken", "Lizard") %>% setNames(c("Chicken", "Lizard")), function(species){
  object = SeuratBootstrap(objectList[[species]])
  object$annotated = gsub("principle", "principal", object$annotated)
  object = FindVariableFeatures(object, selection.method = "vst")
  avgexpr = AvgExpr(object, assay = "RNA", group.by = "annotated", features = object@assays$RNA@var.features)
  
  # principal
  linreg = lm(principal ~ red + green + blue + UV, data = avgexpr)
  principal.coef = summary(linreg)$coefficients
  principal.r.squared = summary(linreg)$r.squared
  
  # accessory
  linreg = lm(accessory ~ red + green + blue + UV, data = avgexpr)
  accessory.coef = summary(linreg)$coefficients
  accessory.r.squared = summary(linreg)$r.squared

  return(list(principal.coef = principal.coef, 
              principal.r.squared = principal.r.squared, 
              accessory.coef = accessory.coef, 
              accessory.r.squared = accessory.r.squared))
})

results = data.frame(chicken.principal = coefficients.list$Chicken$principal.coef[,1], 
                     chicken.accessory = coefficients.list$Chicken$accessory.coef[,1],
                     lizard.principal = coefficients.list$Lizard$principal.coef[,1],
                     lizard.accessory = coefficients.list$Lizard$accessory.coef[,1])

summary = rbind(data.frame(paste0(signif(coefficients.list$Chicken$principal.coef[,1], 2), " (", signif(coefficients.list$Chicken$principal.coef[,4], 2), ")"),
                     paste0(signif(coefficients.list$Chicken$accessory.coef[,1], 2), " (", signif(coefficients.list$Chicken$accessory.coef[,4], 2), ")"), 
                     paste0(signif(coefficients.list$Lizard$principal.coef[,1], 2), " (", signif(coefficients.list$Lizard$principal.coef[,4], 2), ")"), 
                     paste0(signif(coefficients.list$Lizard$accessory.coef[,1], 2), " (", signif(coefficients.list$Lizard$accessory.coef[,4], 2), ")")), 
                            signif(unlist(lapply(coefficients.list, function(x) c(x$principal.r.squared, x$accessory.r.squared))), 2))

colnames(summary) = unlist(lapply(c("Chicken", "Lizard"), function(species) lapply(c("principal", "accessory"), function(type) paste(species, type))))
rownames(summary) = c("Intercept", "red", 'green', 'blue', 'UV', 'Fit (R^2)')
write.csv(summary, "linear_modeling_summary_v2.csv")
```

Bootstrap analysis

```{r}
objectList$Chicken = chick2
coefficients.boot.list = do.call(rbind, lapply(seq_len(10), function(seed) {
  coefficients.list = lapply(c("Chicken", "Lizard") %>% setNames(c("Chicken", "Lizard")), function(species){
    set.seed(seed)
    object = SeuratBootstrap(objectList[[species]])
    object$annotated = gsub("principle", "principal", object$annotated)
    ...
    # rod
    linreg = lm(rod ~ red + green + blue + UV, data = avgexpr)
    rod.coef = summary(linreg)$coefficients
    rod.r.squared = summary(linreg)$r.squared
    ...
    return(list(principal.coef = principal.coef, 
                ...
                accessory.r.squared = accessory.r.squared, 
                rod.coef = rod.coef, 
                rod.r.squared = rod.r.squared))
  })
  
  results = reshape2::melt(as.matrix(data.frame(chicken.principal = coefficients.list$Chicken$principal.coef[,1], 
                                                 chicken.accessory = coefficients.list$Chicken$accessory.coef[,1],
                                                 chicken.rod = coefficients.list$Chicken$rod.coef[,1],
                                                 lizard.principal = coefficients.list$Lizard$principal.coef[,1],
                                                 lizard.accessory = coefficients.list$Lizard$accessory.coef[,1], 
                                                 lizard.rod = coefficients.list$Lizard$rod.coef[,1])))
  
  return(results)
})) %>% setNames(c("type", "predict", "value"))


ggbarplot(coefficients.boot.list, y = "value", x = "type", fill = "type", add = c("jitter", "mean"), facet.by = "predict") + 
      NoLegend()+
      # facet_wrap(~predict)+
...
## Differential gene expression clustering trees
...
# ze_list = lapply(ze_list, function(object) ScaleData(object, features = rownames(object)))

# plotlist = lapply(names(ze_list), function(species) {
#     PseudoBulkPCA(ze_list[[species]], group.by = "annotated", title = species, scale = FALSE, min.pct.expressed = 0, size = 3, shape = 15, cols = pr_palette, label = TRUE) + NoLegend()
# })
...
markersList = list(rownames(pc_corbo), rownames(pc_dev), rownames(pc_adult))

plot_grid(
  plotlist = lapply(seq_along(ze_list), function(index){
    DoHeatmap(DownsampleSeurat(ze_list[[index]], group.by = "annotated", size = 50), features = markersList[[index]], group.by = "annotated") + 
      NoLegend() + 
...
# Zebrafish
ze.genelist = rev(pc_adult$Lyu_adult)
names(ze.genelist) = rev(rownames(pc_adult))

# Cytoskeletal organization in red end and metabolism in UV end
ego1  <- gseGO(geneList    = ze.genelist,
              OrgDb        = org.Dr.eg.db,
              ont          = "BP",
              keyType      = "SYMBOL",
              minGSSize    = 50,
...
gseaplot(ego1, geneSetID = 4, by = "runningScore", title = ego1$Description[4])

dotplot(mutate(ego1, log10pvalue = -log(pvalue, base=10)), color = "NES", x = "log10pvalue", showCategory=20)
```

## All species

```{r}
pca.list = lapply(names(objectList), function(species) PseudoBulkPCA(objectList[[species]], group.by = "annotated", title = species, return.plot = FALSE, nfeatures = 1000, min.pct.expressed = 10))

# Extract top N contributors to PC1
pca.list[[2]]$rotation %>% arrange(PC1) %>% select(PC1)
```
...
# Zebrafish
loadings = pca.list[[1]]$rotation %>% as.data.frame %>% arrange(-PC2)
ze.genelist = loadings$PC2
names(ze.genelist) = rownames(loadings)

# Validate genes
DotPlot2(objectList$Zebrafish, features = c(head(names(ze.genelist), 30), tail(names(ze.genelist), 30)), group.by = "annotated")

# Chicken
loadings = pca.list[[2]]$rotation %>% as.data.frame %>% arrange(-PC1) 
ch.genelist = loadings$PC1
names(ch.genelist) = rownames(loadings)

# Validate genes
DotPlot2(objectList$Chicken, features = head(names(ch.genelist), 30), group.by = "annotated") | DotPlot2(objectList$Chicken, features = tail(names(ch.genelist), 30), group.by = "annotated")

# Lizard
loadings = pca.list[[3]]$rotation %>% as.data.frame %>% arrange(-PC2)
li.genelist = loadings$PC2
names(li.genelist) = rownames(loadings)

# Validate genes
DotPlot2(objectList$Lizard, features = c(head(names(li.genelist), 30), tail(names(li.genelist), 30)), group.by = "annotated")

# Seem to be enriched in metabolic pathways
ego1  <- gseGO(geneList    = li.genelist,
              OrgDb        = org.Gg.eg.db,
              ont          = "BP",
              keyType      = "SYMBOL",
              minGSSize    = 50,
...
gseaplot(ego1, geneSetID = 1, by = "runningScore", title = ego1$Description[2])
...
Are PC loadings consistent between lizard and chicken? 

```{r, fig.height=10, fig.width=10}
# Convert lizard gene list to chicken
# temp = matrix(li.genelist)
# rownames(temp) = names(li.genelist)
li.genelist.subset = li.genelist[names(li.genelist) %in% intersect(names(li.genelist), colnames(li2ch))]
li.genelist.converted = as.matrix(li2ch[,match(names(li.genelist.subset), colnames(li2ch))]) %*% matrix(li.genelist.subset)
li.genelist.converted[li.genelist.converted == 0] = NA

summary.df = data.frame(gene = rownames(li2ch), 
                        Chicken = ch.genelist[match(rownames(li2ch), names(ch.genelist))], 
                        Lizard = li.genelist.converted)
summary.df = na.omit(summary.df)

ScatterPlot(summary.df, "Chicken", "Lizard", labels = "gene", max.overlaps = 10)
```

# Export to python (e.g. for SAMap, scANVI, etc.)
...
```{r}
# Remove double cones for control
types.remove = c("opn1mw4/opn1lw1", "principle", "accessory", "full", "principal") 

# Writing to h5ad
for(seed in 1:3){
  lapply(names(objectList), function(species){ 
    SeuratToH5ad(objectList[[species]], 
                 filepath = paste0("../../Species_Objects/Cone_Objects/bootstrap/", species, "_single_", seed, ".h5ad"), 
                 genes.remove = NULL, 
                 types.remove = types.remove, 
                 downsample = 100, 
                 seed = seed)
  })
}
...
# Writing to h5ad
dev_objects = lapply(4:6, function(seed){
  lapply(names(objectList), function(species){ 
    SeuratToH5ad(objectList[[species]], 
                 filepath = paste0("../../Species_Objects/Cone_Objects/bootstrap_lyu/", species, "_single_", seed, ".h5ad"), 
                 genes.remove = NULL, 
                 types.remove = types.remove, 
                 downsample = 100, 
                 seed = seed)
...
# Writing to h5ad
adult_objects = lapply(7:9, function(seed){
  lapply(names(objectList), function(species){ 
    SeuratToH5ad(objectList[[species]], 
                 filepath = paste0("../../Species_Objects/Cone_Objects/bootstrap_lyu_adult/", species, "_single_", seed, ".h5ad"), 
                 genes.remove = NULL, 
                 types.remove = types.remove, 
                 downsample = 100, 
                 seed = seed)
...
adult_objects = lapply(7:9, function(seed){
  lapply(names(objectList), function(species){ 
    SeuratToH5ad(objectList[[species]], 
                 filepath = paste0("../../Species_Objects/Cone_Objects/bootstrap_lyu_adult_converted/", species, "_single_", seed, ".h5ad"), 
                 genes.remove = NULL, 
                 types.remove = types.remove, 
                 downsample = 100, 
                 seed = seed)
...
## Corbo
objectList$Zebrafish = readRDS("../../Species_Objects/Cone_Objects/Zebrafish_clean_v1.rds")
objectList$Zebrafish$animal = 'S1'

# Writing to h5ad
corbo_objects = lapply(1:3, function(seed){
  lapply(names(objectList), function(species){ 
    SeuratToH5ad(objectList[[species]], 
                 filepath = paste0("../../Species_Objects/Cone_Objects/bootstrap/", species, "_double_", seed, ".h5ad"), 
                 genes.remove = NULL, 
                 types.remove = types.remove, 
                 downsample = 100, 
                 seed = seed)
...
dev_objects = lapply(4:6, function(seed){
  lapply(names(objectList), function(species){ 
    SeuratToH5ad(objectList[[species]], 
                 filepath = paste0("../../Species_Objects/Cone_Objects/bootstrap_lyu/", species, "_double_", seed, ".h5ad"), 
                 genes.remove = NULL, 
                 types.remove = types.remove, 
                 downsample = 100, 
                 seed = seed)
...
## Lyu developing converted
objectList$Zebrafish = readRDS("../../Species_Objects/Cone_Objects/Lyu_Zebrafish_PR2.rds")
# objectList$Zebrafish@misc$converted.feature.names = rownames(ze_adult_con)
objectList$Zebrafish = RenameFeatures(objectList$Zebrafish, new.names = objectList$Zebrafish@misc$converted.feature.names)

# Writing to h5ad
dev_objects = lapply(4:6, function(seed){
  lapply(names(objectList), function(species){ 
    SeuratToH5ad(objectList[[species]], 
                 filepath = paste0("../../Species_Objects/Cone_Objects/bootstrap_lyu_converted/", species, "_double_", seed, ".h5ad"), 
                 genes.remove = NULL, 
                 types.remove = types.remove, 
                 downsample = 100, 
                 seed = seed)
...
## Lyu adult
objectList$Zebrafish = readRDS("../../Species_Objects/Cone_Objects/Lyu_Zebrafish_PR_adult.rds") # V2 has the converted names which results in worse samap alignment

# Writing to h5ad
adult_objects = lapply(7:9, function(seed){
  lapply(names(objectList), function(species){ 
    SeuratToH5ad(objectList[[species]], 
                 filepath = paste0("../../Species_Objects/Cone_Objects/bootstrap_lyu_adult/", species, "_double_", seed, ".h5ad"), 
                 genes.remove = NULL, 
                 types.remove = types.remove, 
                 downsample = 100, 
                 seed = seed)
...
adult_objects = lapply(7:9, function(seed){
  lapply(names(objectList), function(species){ 
    SeuratToH5ad(objectList[[species]], 
                 filepath = paste0("../../Species_Objects/Cone_Objects/bootstrap_lyu_adult/", species, "_all_", seed, ".h5ad"), 
                 genes.remove = NULL, 
                 types.remove = c('full'), 
                 downsample = 100, 
                 seed = seed)
...
adult_objects = lapply(7:9, function(seed){
  lapply(names(objectList), function(species){ 
    SeuratToH5ad(objectList[[species]], 
                 filepath = paste0("../../Species_Objects/Cone_Objects/bootstrap_lyu_adult_converted/", species, "_double_", seed, ".h5ad"), 
                 genes.remove = NULL, 
                 types.remove = types.remove, 
                 downsample = 100, 
                 seed = seed)
...
* opossum was realigned to NCBI and cleaned up
* squirrel instead of rat
...
# updated_objects = lapply(c("Zebrafish", "ZebrafishAdult", "Squirrel", "Human"), function(species){ 
#     SeuratToH5ad(updatedList[[species]], 
#                  filepath = paste0("../../Species_Objects/Cone_Objects/updated/", species, "_single.h5ad"), 
#                  genes.remove = NULL, 
#                  types.remove = c("full", "opn1mw4/opn1lw1", "principal", "accessory"), 
...
```{r}
# Remove double cones for control
types.remove = c("opn1mw4/opn1lw1", "principle", "accessory") 

# Remove opsins
genes.remove = setdiff(Genes(pr_annotation), "THRB") # Keep THRB since its not an opsin

# Writing to h5ad
lapply(names(objectList[names(objectList) %in% c("Chicken")]), function(species){ # "Zebrafish", "Opossum", "Lizard"
  
  # DCs removed
  SeuratToH5ad(objectList[[species]], 
               filepath = paste0("../../Species_Objects/Cone_Objects/", species, "_ds_v4.h5ad"), 
               genes.remove = NULL, 
               types.remove = types.remove, 
               downsample = 100)
  ...
  # DCs included
  SeuratToH5ad(objectList[[species]], 
               filepath = paste0("../../Species_Objects/Cone_Objects/", species, "_inc_v4.h5ad"), 
               genes.remove = NULL, 
               types.remove = NULL, 
               downsample = 100)
})
...
# Writing to h5ad
lapply(names(objectList[names(objectList) %in% c("Rat", "Human")]), function(species){
  
  SeuratToH5ad(objectList[[species]], 
               filepath = paste0("../../Species_Objects/Cone_Objects/", species, "_ds_v3.h5ad"), 
               genes.remove = NULL, 
               types.remove = NULL, 
               downsample = 100)
})
...
```{r}
# Export new Lyu zebrafish data
SeuratToH5ad(objectList$Zebrafish, 
               filepath = paste0("../../Species_Objects/Cone_Objects/Zebrafish_ds_v4.h5ad"), 
               genes.remove = NULL, 
               # types.remove = types.remove, 
               downsample = 100)

# Export new Lyu zebrafish adult data
SeuratToH5ad(objectList$Zebrafish, 
               filepath = paste0("../../Species_Objects/Cone_Objects/Zebrafish_ds_v5.h5ad"), 
               genes.remove = NULL, 
               types.remove = "opn1mw4/opn1lw1",
               downsample = 100)
```
...
SeuratToH5ad(objectList$Chicken, 
             filepath = paste0("../../Species_Objects/Cone_Objects/Chicken_ds_v5.h5ad"), 
             genes.remove = NULL, 
             types.remove = c("full", "principle", "accessory"),
             downsample = 100, 
             seed = 1)

SeuratToH5ad(objectList$Chicken, 
             filepath = paste0("../../Species_Objects/Cone_Objects/Chicken_ds_v5_2.h5ad"), 
             genes.remove = NULL, 
             types.remove = c("full", "principle", "accessory"),
             downsample = 100, 
             seed = 3)
...
SeuratToH5ad(objectList$Chicken, 
             filepath = paste0("../../Species_Objects/Cone_Objects/Chicken_ds_v5_3.h5ad"), 
             genes.remove = NULL, 
             types.remove = c("full", "principle", "accessory"),
             downsample = 100, 
             seed = 4)
...
# With double cones
SeuratToH5ad(objectList$Chicken, 
             filepath = paste0("../../Species_Objects/Cone_Objects/Chicken_inc_v5.h5ad"), 
             genes.remove = NULL, 
             types.remove = c("full"),
             downsample = 100, 
             seed = 1)
...
```{r}
# Zebrafish without doublets
SeuratToH5ad(subset(objectList$Zebrafish, DF.classifications == "Singlet"), 
               filepath = paste0("../../Species_Objects/Cone_Objects/Zebrafish_ds_v3_singlet.h5ad"), 
               genes.remove = NULL, 
               types.remove = types.remove, 
               downsample = 100)

# Zebrafish with SoupX-corrected counts
SeuratToH5ad(readRDS("../../Species_Objects/Cone_Objects/Zebrafish_clean_v2_soupx.rds"), 
               filepath = paste0("../../Species_Objects/Cone_Objects/Zebrafish_ds_v3_soupx.h5ad"), 
               genes.remove = NULL, 
               types.remove = types.remove, 
               downsample = 100)

# Zebrafish with blue opsin reduced
ze_pr = objectList$Zebrafish
ze_pr@assays$RNA@counts["opn1sw2", ze_pr$annotated != "blue"] = 0
ze_pr@assays$RNA@counts["opn1sw1", ze_pr$annotated != "UV"] = 0
# ze_pr@assays$RNA@data["opn1sw2", ze_pr$annotated != "blue"] = 0
# ze_pr@assays$RNA@data["opn1sw1", ze_pr$annotated != "UV"] = 0
AnnotatedUmap(ze_pr, pr_annotation, group.by = "annotated", color.clusters.by = "annotated")
SeuratToH5ad(ze_pr, filepath = "../../Species_Objects/Cone_Objects/Zebrafish_blue_UV_cleaned.h5ad", genes.remove = NULL, types.remove = c("opn1mw4/opn1lw1"), downsample = 100)
```
...
```{r, eval = params$save}
# mclapply(seq_along(objectList), function(i) SmartSaveRDS(objectList[[i]], paste0("../../Species_Objects/Cone_Objects/", names(objectList)[i], "_clean_v3.rds")), mc.cores = params$cores)
```
...
```{r, eval = FALSE}
# objectList = mclapply(metadata$speciesList, function(species) readRDS(paste0("../../Species_Objects/Cone_Objects/", species, "_clean_v3.rds")), mc.cores = params$cores)
# names(objectList) = metadata$speciesList
```
...
# lizard_other = DownsampleSeurat(subset(lizard_full, seurat_clusters != 0 & cell_class %in% c("HC", "AC")), group.by = "cell_class", size = 100)
# lizard_other$annotated = lizard_other$cell_class
# AnnotatedDotPlot(lizard_other, major_annotation, group.by = "cell_class", color.clusters.by = "cell_class", 
#                  features = rev(Genes(major_annotation)), color.genes = FALSE) + coord_flip() + RotatedAxis()
# lizard_control = merge(objectList$Lizard, lizard_other)
# SeuratToH5ad(lizard_control, 
#                filepath = paste0("../../Species_Objects/Cone_Objects/Lizard_swap_control.h5ad"), 
#                genes.remove = NULL, 
...
# chick_other = DownsampleSeurat(subset(chick.new, cell_class %in% c("HC", "AC")), group.by = "cell_class", size = 100)
...
#                  features = rev(Genes(major_annotation)), color.genes = FALSE) + coord_flip() + RotatedAxis()
# SeuratToH5ad(chick_control, 
#                filepath = paste0("../../Species_Objects/Cone_Objects/Chicken_swap_control.h5ad"), 
#                genes.remove = NULL, 
...
AnnotatePlot(DotPlot2(chick_other, features = rev(Genes(major_annotation))),
             major_annotation, color_genes_x = TRUE)
# DotPlot2(chick_other, features = rev(Genes(major_annotation)), group.by = "Cluster")
# AnnotatedDotPlot(chick_other, major_annotation, group.by = "Cluster", color.clusters.by = "cell_class",
#                  features = rev(Genes(major_annotation)), color.genes = TRUE) + coord_flip() + RotatedAxis()
...
# SAMapHeatmap(objectList, 
#              top.ortho.markers$principle %>% unique, 
#              species.use = c("Zebrafish", "Chicken", "Lizard", "Opossum", "Rat", "Human"), #names(species_ids), 
#              types.use = c("principle", "accessory", "red", "green", "blue", "UV", "rod"), 
#              show_heatmap_legend = TRUE, 
#              width = unit(4, "in"))
...
```{r}
# Idents(objectList$Chicken) = "annotated"
...
# Zebrafish ensembl
# ze2ch = BlastOrthologyTable(file1 = "maps/zech/ch_to_ze.txt", 
...
# edge.1 <- setNames(as.data.frame(get.edgelist(graph_from_incidence_matrix(ze2ch))), c("Chicken", "Zebrafish"))
# edge.2 <- setNames(as.data.frame(get.edgelist(graph_from_incidence_matrix(li2ch))), c("Chicken", "Lizard"))
# 
# ortholog.table = SmartMerge(list(edge.1, edge.2), by = "Chicken")
...
# SAMapHeatmap(objectList, 
#              ortholog.table[ortholog.table$Chicken %in% gene.summary$rod[[5]]$Chicken,], 
#              species.use = c("Chicken", "Zebrafish", "Lizard"))
```
...
## Opossum, human, and rat works

```{r}
op2hs = BlastOrthologyTable(file1 = "maps/ophs/hs_to_op.txt", 
                            file2 = "maps/ophs/op_to_hs.txt", 
                            keyfile1 = "Keys/Human_key.csv",
                            keyfile2 = "Keys/Opossum_key.csv")

rn2hs = BlastOrthologyTable(file1 = "maps/rnhs/hs_to_rn.txt", 
                            file2 = "maps/rnhs/rn_to_hs.txt", 
                            keyfile1 = "Keys/Human_key.csv",
                            keyfile2 = "Keys/Rat_key.csv")
colnames(rn2hs) = toupper(colnames(rn2hs))
objectList$Human$species = "Human"
objectList$Rat$species = "Rat"
```

```{r, fig.height=3, fig.width=8}
ortho = OrthotypeAnalysis2(objectList$Human, 
                           objectList[c("Rat", "Opossum")], list(rn2hs, op2hs), 
                           downsample = 100, 
                           types.use = c("red", "green", "blue", "UV", "rod"), 
                           cluster_resolution = 0.5)
DimPlot(ortho, group.by = "species") | DimPlot(ortho, label = TRUE) + NoLegend() | DimPlot(ortho, group.by = "annotated", label = TRUE) + NoLegend()
...
# gene_pairs = read.csv("samap/ze_ch_li_op_rn_hs_gene_pairs.csv", row.names = 1)
# gene_pairs = read.csv("samap/controlfiles4_gp.csv", row.names = 1)
gene_pairs = read.csv("MappingTable/zechlisqhs_genepairs.csv", row.names = 1)
# chicken.genes = ExtractString(gene_pairs$ch_UV.ze_UV, "ch_", ";")
# zebrafish.genes = ExtractString(gene_pairs$ch_UV.ze_UV, "ze_")
...
# Reading in h5ad fails with Error: Missing required datasets 'levels' and 'values'
# library(SeuratDisk)
# Convert("samap/controlfiles4.h5ad", dest = "h5seurat", overwrite = TRUE)
# sm <- LoadH5Seurat("samap/controlfiles4.h5seurat")
# sm

sm_umap = fread('MappingTable/zechlisqhs_single_res_0_leiden.csv', header = TRUE)[,-1] %>% 
  as.data.frame %>% 
  setNames(c("UMAP_1", "UMAP_2", "annotated", "species", "leiden"))

# sm_umap$orthotype = gsub('0', 'rod', gsub('1', 'red', gsub('2', 'UV', gsub('3', 'blue', gsub('4', 'green', sm_umap$leiden)))))
sm_umap$leiden = as.character(sm_umap$leiden)

# Overlap analysis
sm_umap$species_annotated = paste0(sm_umap$species, "_", sm_umap$annotated)
jaccard_matrix = do.call(rbind, lapply(unique(sm_umap$species), function(species_) {
  sm_umap = subset(sm_umap, species %in% species_)
  JSMatrix(table(sm_umap$species_annotated, sm_umap$leiden))
}))

# Ordered by species
ancestral_types =c("red", "green", "blue", "UV", "rod")
type_order = as.vector(outer(names(species_palette2), ancestral_types, function(x, y) paste0(x, '_', y)))
species_order = as.vector(outer(ancestral_types, names(species_palette2), function(y, x) paste0(x, '_', y)))
# rownames(jaccard_matrix) = factor(rownames(jaccard_matrix), levels = type_order)

plt.leiden = plot_grid(
  TitlePlot(PrettyUmap(sm_umap, group.by = "leiden", label = TRUE, show.legend = FALSE, angle = -pi/2,
                       cols = c('turquoise', 'gold', 'orangered', 'purple', 'grey')),
            title = 'Unsupervised clusters'),
  # NA,
  TitlePlot(JSHeatmap(t(jaccard_matrix),
          column.order = species_order,
          row.order = c(0,2,4,1,3),
          max.value = 0.5,
          stagger.threshold = 0.05) +
          scale_fill_gradient2(low = "white", mid = 'orangered', high = "darkred", midpoint = 0.4, limits = c(0,0.6), na.value = "darkred"),
    title = "Correspondence to true clusters\n"),
  nrow = 1, rel_widths = c(0.6,1), labels = c('c', 'd'), label_size = 20
)
plt.leiden
```
...
# SAMapAlignmentHeatmap(MeanTable("MappingTable/chli_double_res"), cols = c("white", "#fbd9d3", "#ffb09c", "red", "darkred"))
# SAMapAlignmentHeatmap(MeanTable("MappingTable/chli_swapch_res"), cols = c("white", "#fbd9d3", "#ffb09c", "red", "darkred"))
# SAMapAlignmentHeatmap(MeanTable("MappingTable/chli_swapli_res"), cols = c("white", "#fbd9d3", "#ffb09c", "red", "darkred"))
...
# gene_pairs = read.csv("MappingTable/chli_swapch_gene_pairs.csv", row.names = 1)
# type.list1 = GetTypeList(gene_pairs, PHOTORECEPTORS, species_ids)
# 
# gene_pairs = read.csv("MappingTable/chli_swapli_gene_pairs.csv", row.names = 1)
# type.list2 = GetTypeList(gene_pairs, PHOTORECEPTORS, species_ids)
...
```{r}
# principal_summary = rbind(type.list1$ch$red$li$principal, 
#                           type.list2$ch$principal$li$red) %>% unique
# principal_summary$ident = paste0(principal_summary$Chicken, '-', principal_summary$Lizard)
# 
# accessory_summary = rbind(type.list1$ch$red$li$accessory, 
#                           type.list2$ch$accessory$li$red) %>% unique
...
# gene_summary = rbind(a2a_summary, p2p_summary)
#                      #accessory_summary, principal_summary, both)
# gene_summary$row_annotation = c(rep("accessory markers", nrow(a2a_summary)), 
#                                 rep("principal markers", nrow(p2p_summary)))
#                                 # rep("accessory-red", nrow(accessory_summary)), 
#                                 # rep("principal-red", nrow(principal_summary)),
#                                 # rep("all", nrow(both)))
```

```{r, fig.height=6, fig.width=8}
type_palette = pr_palette
...
## All photoreceptors heatmap

Figure for Tom's mini review

Extract genes for single cones and rods

```{r, fig.height=25, fig.width=13}
# This integration included double cones but not mammals
# gene_pairs = read.csv("MappingTable/zechli_gene_pairs_0.csv", row.names = 1)

# This has all photoreceptor types and all five species
gene_pairs = read.csv("MappingTable/zechlisqhs_double_genepairs.csv", row.names = 1)

PHOTORECEPTORS = setNames(c('red', 'green', 'blue', 'UV', 'principal', 'accessory', 'rod'), 
                          c('red', 'green', 'blue', 'UV', 'principal', 'accessory', 'rod'))
species_ids = sort(setNames(c('ze', 'ch', 'li', 'sq', 'hs'), 
                            c('Zebrafish', 'Chicken', 'Lizard', 'Squirrel', 'Human')))
...
SAMapHeatmap(objectList, 
             sm_gene_table, 
             species.use = c("Zebrafish", "Chicken", "Lizard", "Squirrel", "Human"), 
             types.use = c("red", "green", "blue", "UV", 'principal', 'accessory', "rod"), 
             show_heatmap_legend = FALSE, 
             show_row_names = TRUE,
             show_column_names = FALSE, 
             width = unit(5, "in"))
...
Principal:
tenm3/TENM2/TENM2/TENM2/TENM2
thrb/THRB/THRB/THRB/THRB
elfn1b/ELFN1/ELFN1/ELFN1/ELFN1
NA/GUCA1C/GUCA1C/NA/NA
rbp4l/RBP4/LOC132768105/NA/NA
ckbb/CKB/CKB/CKB/CKB
kif1b/KIF1B/KIF1B/KIF1B/KIF1B
sh3gl2a/SH3GL2/SH3GL2/SH3GL2/SH3GL2

Accessory:
lingo1a/LINGO1/LINGO2/NA/NA
TMEM132C/TMEM132C/TMEM132C/NA/NA
stbd1/STBD1/STBD1/STBD1/STBD1
mylka/MYLK/MYLK/MYLK/MYLK
SPHKAP/SPHKAP/SPHKAP/SPHKAP/SPHKAP
NA/MPDZ/MPDZ/MPDZ/MPDZ
plxna1a/PLXNA1/PLXNA1/PLXNA1/PLXNA1
map2/MAP2/MAP2/MAP2/MAP2
NA/AKAP9/AKAP9/AKAP9/AKAP9
sema6dl/SEMA6D/SEMA6D/SEMA6D/SEMA6D
...
graph.list = c(ze2ch, li2ch, sq2ch, hs2ch)

## Principal
lapply(graph.list, function(graph) ConvertGeneSymbolsFromMatrix(t(graph), c('ELFN1', 'GUCA1C', 'RBP4', 'OPN1LW', 'SH3GL2', 'KIF1B', 'CKB')))

## Accessory
lapply(graph.list, function(graph) ConvertGeneSymbolsFromMatrix(t(graph), c('LINGO1', 'TMEM132C', 'STBD1', 'MYLK', 'SPHKAP', 'MPDZ', 'PLXNA1', 'MAP2', 'AKAP9', 'SEMA6D')))
```

Get the original genes from figure 2
...
sm_gene_table_no_opsin = subset(sm_gene_table, !startsWith(Chicken, 'OPN') & !startsWith(Chicken, 'RHO'))
sm_gene_table_no_opsin

filtered_df <- sm_gene_table_no_opsin %>%
  group_by(row_annotation) %>%
  slice_head(n = 10) %>%
  ungroup()

filtered_df

# Add in principal and accessory

data_string <- "tenm3/TENM2/TENM2/TENM2/TENM2
thrb/THRB/THRB/THRB/THRB
elfn1b/ELFN1/ELFN1/ELFN1/ELFN1
NA/GUCA1C/GUCA1C/NA/NA
rbp4l/RBP4/LOC132768105/NA/NA
...
df <- read.delim(text = data_string, header = FALSE, sep = "/")

# Add column names if needed
colnames(df) <- c("Zebrafish", "Chicken", "Lizard", "Squirrel", "Human")
df$row_annotation = c(rep('principal', 8), rep('accessory', 10))

combined.df = rbind(filtered_df[,c(5,1,3,4,2,6)],df)
combined.df = combined.df %>% arrange(factor(row_annotation, levels = c('red', 'green', 'blue', 'UV', 'principal', 'accessory', 'rod')))

pdf("figures/all_pr_top10.pdf", height=10, width=10)
SAMapHeatmap(objectList, 
             combined.df, 
             species.use = c("Zebrafish", "Chicken", "Lizard", "Squirrel", "Human"), 
             types.use = c("red", "green", "blue", "UV", 'principal', 'accessory', "rod"), 
...
write.csv(dat, 'figures/all_pr_top10.csv')
```
...
```{r}
sm_expr = as.matrix(fread("samap/controlfiles4_translation.csv"), rownames=1)
species = str_split_fixed(colnames(sm_expr), "_", 2)[,1]
rownames(sm_expr) = str_split_fixed(rownames(sm_expr), "_", 2)[,2]
colnames(sm_expr) = str_split_fixed(colnames(sm_expr), "_", 2)[,2]
sm_seurat = CreateSeuratObject(sm_expr)
sm_seurat$species = species

# Add metadata
sm_seurat = MapBack(sm_seurat, objectList$Chicken, group.by = "annotated")
sm_seurat$ch.annotated = sm_seurat$mapped.back

sm_seurat = MapBack(sm_seurat, objectList$Lizard, group.by = "annotated")
sm_seurat$li.annotated = sm_seurat$mapped.back

sm_seurat = MapBack(sm_seurat, objectList$Zebrafish, group.by = "annotated")
sm_seurat$ze.annotated = sm_seurat$mapped.back

sm_seurat$annotated = gsub("NA", "", paste0(sm_seurat$ch.annotated, sm_seurat$li.annotated, sm_seurat$ze.annotated))

# Checks
table(sm_seurat$annotated)
table(sm_seurat$species)

# Define clusters
sm_seurat$species_annotated = paste0(sm_seurat$species, "_", sm_seurat$annotated)
```

## Feature selection

```{r}
top2000at5 = SelectFeatures(sm_seurat, nfeatures = 2000, min.pct.expressed = 5, group.by = "species_annotated")
top1000at5 = SelectFeatures(sm_seurat, nfeatures = 1000, min.pct.expressed = 5, group.by = "species_annotated")
top1000at10 = SelectFeatures(sm_seurat, nfeatures = 1000, min.pct.expressed = 10, group.by = "species_annotated")
top4000at5 = SelectFeatures(sm_seurat, nfeatures = 4000, min.pct.expressed = 5, group.by = "species_annotated")
top4000at10 = SelectFeatures(sm_seurat, nfeatures = 4000, min.pct.expressed = 10, group.by = "species_annotated")

# All five opsins are present
top1000at10[startsWith(top1000at10, "OPN")]

write.csv(top2000at5, "samap/top2000at5.csv", row.names = FALSE)
write.csv(top1000at5, "samap/top1000at5.csv", row.names = FALSE)
write.csv(top1000at10, "samap/top1000at10.csv", row.names = FALSE)
write.csv(top4000at5, "samap/top4000at5.csv", row.names = FALSE)
write.csv(top4000at10, "samap/top4000at10.csv", row.names = FALSE)
```

```{r}
sm_expr = as.matrix(fread("samap/expfiles4_translation.csv"), rownames=1)
species = str_split_fixed(colnames(sm_expr), "_", 2)[,1]
rownames(sm_expr) = str_split_fixed(rownames(sm_expr), "_", 2)[,2]
colnames(sm_expr) = str_split_fixed(colnames(sm_expr), "_", 2)[,2]
sm_seurat = CreateSeuratObject(sm_expr)
...
sm_seurat = MapBack(sm_seurat, ze_dev, group.by = "annotated")
sm_seurat$ze.annotated = sm_seurat$mapped.back
...
top10000at1 = SelectFeatures(sm_seurat, nfeatures = 10000, min.pct.expressed = 1, group.by = "species_annotated")
top2000at5 = SelectFeatures(sm_seurat, nfeatures = 2000, min.pct.expressed = 5, group.by = "species_annotated")
top1000at5 = SelectFeatures(sm_seurat, nfeatures = 1000, min.pct.expressed = 5, group.by = "species_annotated")
top1000at10 = SelectFeatures(sm_seurat, nfeatures = 1000, min.pct.expressed = 10, group.by = "species_annotated")
top4000at5 = SelectFeatures(sm_seurat, nfeatures = 4000, min.pct.expressed = 5, group.by = "species_annotated")
top4000at10 = SelectFeatures(sm_seurat, nfeatures = 4000, min.pct.expressed = 10, group.by = "species_annotated")
top5000at5 = SelectFeatures(sm_seurat, nfeatures = 5000, min.pct.expressed = 5, group.by = "species_annotated")
top5000at10 = SelectFeatures(sm_seurat, nfeatures = 5000, min.pct.expressed = 10, group.by = "species_annotated")

# All five opsins are present
top1000at10[startsWith(top1000at10, "OPN")]

write.csv(top10000at1, "samap/top10000at1_exp.csv", row.names = FALSE)
write.csv(top2000at5, "samap/top2000at5_exp.csv", row.names = FALSE)
write.csv(top1000at5, "samap/top1000at5_exp.csv", row.names = FALSE)
write.csv(top1000at10, "samap/top1000at10_exp.csv", row.names = FALSE)
write.csv(top4000at5, "samap/top4000at5_exp.csv", row.names = FALSE)
write.csv(top4000at10, "samap/top4000at10_exp.csv", row.names = FALSE)
write.csv(top5000at5, "samap/top5000at5_exp.csv", row.names = FALSE)
write.csv(top5000at10, "samap/top5000at10_exp.csv", row.names = FALSE)
```


```{r}
ch.expressed = SelectFeatures(subset(sm_seurat, species == 'ch'), nfeatures = 100000, min.pct.expressed = 10, group.by = "species_annotated")
li.expressed = SelectFeatures(subset(sm_seurat, species == 'li'), nfeatures = 100000, min.pct.expressed = 10, group.by = "species_annotated")
ze.expressed = SelectFeatures(subset(sm_seurat, species == 'ze'), nfeatures = 100000, min.pct.expressed = 10, group.by = "species_annotated")

intersection10 = Reduce(intersect, list(ch.expressed, li.expressed, ze.expressed))
write.csv(intersection10, "samap/features_intersection10.csv", row.names = FALSE)

ch.expressed = SelectFeatures(subset(sm_seurat, species == 'ch'), nfeatures = 100000, min.pct.expressed = 5, group.by = "species_annotated")
li.expressed = SelectFeatures(subset(sm_seurat, species == 'li'), nfeatures = 100000, min.pct.expressed = 5, group.by = "species_annotated")
ze.expressed = SelectFeatures(subset(sm_seurat, species == 'ze'), nfeatures = 100000, min.pct.expressed = 5, group.by = "species_annotated")

intersection5 = Reduce(intersect, list(ch.expressed, li.expressed, ze.expressed))
write.csv(intersection5, "samap/features_intersection5.csv", row.names = FALSE)
```

Just zebrafish since a similar issue was occurring with that PCA (some noisy lowly expressed genes)

```{r}
ze.expressed = SelectFeatures(subset(sm_seurat, species == 'ze'), nfeatures = 100000, min.pct.expressed = 1, group.by = "species_annotated")
write.csv(ze.expressed, 'samap/features_zebrafish1.csv', row.names = FALSE)

ze.expressed = SelectFeatures(subset(sm_seurat, species == 'ze'), nfeatures = 100000, min.pct.expressed = 2, group.by = "species_annotated")
write.csv(ze.expressed, 'samap/features_zebrafish2.csv', row.names = FALSE)

ze.expressed = SelectFeatures(subset(sm_seurat, species == 'ze'), nfeatures = 100000, min.pct.expressed = 5, group.by = "species_annotated")
write.csv(ze.expressed, 'samap/features_zebrafish5.csv', row.names = FALSE)

# Since we sub
ze_dev.pct = SelectFeatures(ze_dev, nfeatures = 100000, min.pct.expressed = 1, group.by = "annotated", return.pct = TRUE)
ze.pct = SelectFeatures(subset(sm_seurat, species == 'ze'), nfeatures = 100000, min.pct.expressed = 1, group.by = "species_annotated", return.pct = TRUE)
```

## Pseudobulk PCA

```{r}
Idents(sm_seurat) = "species_annotated"
VlnPlot(sm_seurat, group.by = "species_annotated", features = c("nFeature_RNA", "nCount_RNA"))

# Pseudobulk
features.use = rownames(sm_seurat)
PseudoBulkPCA(subset(sm_seurat, species == "ze"), features = features.use, group.by = "species_annotated")
PseudoBulkPCA(subset(sm_seurat, species == "ze"), group.by = "species_annotated", min.pct.expressed = 25)
PseudoBulkPCA(subset(sm_seurat, species == "ze"), group.by = "species_annotated", features = features.use, scale = FALSE)
PseudoBulkPCA(subset(sm_seurat, species == "ch"), group.by = "species_annotated", min.pct.expressed = 5)
PseudoBulkPCA(subset(sm_seurat, species == "li"), group.by = "species_annotated", min.pct.expressed = 5)

# Tests with original zebrafish
PseudoBulkPCA(objectList$Zebrafish, group.by = "annotated", min.pct.expressed = 0)

# This works
PseudoBulkPCA(objectList$Zebrafish, group.by = "annotated", min.pct.expressed = 1)

# This also works
PseudoBulkPCA(objectList$Zebrafish, group.by = "annotated", min.pct.expressed = 0, scale = FALSE)
```


```{r}
# Zebrafish behaves weird already before even embedding in the same space as lizard and chicken
DotPlot2(sm_seurat, features = c("OPN1LW", "OPN1MSW", "OPN2SW", "OPN1SW", "RHO"))
```

What drives ze UV similarity to red in PC2? 

```{r}
ze_pca = PseudoBulkPCA(subset(sm_seurat, species == "ze"), features = features.use, group.by = "species_annotated", return.plot = FALSE)
ze_pc2 = PcaContribution(ze_pca, 'PC2')

DotPlot2(subset(sm_seurat, species == "ze"), features = c(head(names(ze_pc2)), tail(names(ze_pc2))))

# PCA is attending to wrong stuff
DotPlot2(objectList$Zebrafish, features = c("casp8", "tbcc", "npc1", "hagh", "pnkd"), group.by = "annotated")
```

```{r}
ze_pca = PseudoBulkPCA(subset(sm_seurat, species == "ze"), features = features.use, group.by = "species_annotated", return.plot = FALSE, scale = FALSE)
ze_pc2 = PcaContribution(ze_pca, 'PC2')
...
```{r}
ze_pca = PseudoBulkPCA(subset(sm_seurat, species == "ze"), group.by = "species_annotated", return.plot = FALSE, min.pct.expressed = 25)
ze_pc2 = PcaContribution(ze_pca, 'PC2')
...
PCA is dominated by lowly expressed genes

```{r}
ze_pca = PseudoBulkPCA(objectList$Zebrafish, group.by = "annotated", return.plot = FALSE, min.pct.expressed = 0)
ze_pc2 = PcaContribution(ze_pca, 'PC2')

DotPlot2(objectList$Zebrafish, features = c(head(names(ze_pc2)), tail(names(ze_pc2))), group.by = "annotated")
```

```{r}
ze_pca = PseudoBulkPCA(objectList$Zebrafish, group.by = "annotated", return.plot = FALSE, min.pct.expressed = 1)
ze_pc2 = PcaContribution(ze_pca, 'PC2')
...
```{r}
ze_pca = PseudoBulkPCA(objectList$Zebrafish, group.by = "annotated", return.plot = FALSE, min.pct.expressed = 15)
ze_pc2 = PcaContribution(ze_pca, 'PC2')
...
```{r}
ze_pca = PseudoBulkPCA(objectList$Zebrafish, group.by = "annotated", return.plot = FALSE, scale = FALSE)
ze_pc2 = PcaContribution(ze_pca, 'PC2')

DotPlot2(objectList$Zebrafish, features = c(head(names(ze_pc2)), tail(names(ze_pc2))), group.by = "annotated")
```

## Quantile normalization doesn't help
...
sm_avgexpr = AvgExpr(sm_seurat, assay = "RNA", group.by = "species_annotated")
sm_avgexpr_qn <- quantile_normalisation(sm_avgexpr)

boxplot(sm_avgexpr+1, log = 'y')
boxplot(sm_avgexpr_qn+1, log = 'y')

PseudoBulkPCA(sm_avgexpr)
PseudoBulkPCA(sm_avgexpr_qn)
```

## Within-species scaling works

```{r, fig.height=4, fig.width=4}
ch.scale = t(scale(t(sm_avgexpr[,startsWith(colnames(sm_avgexpr), "ch")])))
li.scale = t(scale(t(sm_avgexpr[,startsWith(colnames(sm_avgexpr), "li")])))
ze.scale = t(scale(t(sm_avgexpr[,startsWith(colnames(sm_avgexpr), "ze")])))

sm_avgexpr_ws = cbind(ch.scale, li.scale, ze.scale)
PseudoBulkPCA(sm_avgexpr_ws, binary = 0)
```

## Combat batch correction

```{r}
batch = str_split_fixed(colnames(sm_avgexpr))
# modcombat = model.matrix(~1, data=pheno)
# modcancer = model.matrix(~cancer, data=pheno)
combat_edata = ComBat(dat=sm_avgexpr, batch=batch)
```

## Integrate and inspect

```{r, fig.height=7, fig.width=5}
sm_seurat = ClusterSeurat(sm_seurat, integrate.by = "species", cluster_resolution = 0.2)
DimPlot(sm_seurat, group.by = c('species', 'annotated', 'seurat_clusters'), ncol = 1, shuffle = TRUE)
```
```{r}
markers0_1 = FindMarkers(sm_seurat, ident.1 = 0, ident.2 = 1)
DotPlot2(sm_seurat, features = rownames(markers0_1)[1:5])

# Splitting by quality differences for some reason; cluster 0 is higher quality
FeaturePlot(sm_seurat, features = "nFeature_RNA")
```
...
# VlnPlot(objectList$Zebrafish, features = c("zeb2a", "zeb2b", "calb1"), group.by = "annotated")
```
...
## Principal and accessory markers

```{r, fig.height=4, fig.width=25}
chicken = readRDS('../../Full_Objects/Chicken_full_v1.rds')
DotPlot2(chicken, features = c('CALB1', 'CALB2', 'DSCAM', 'SOX5'), group.by = 'annotated')
```

# IHC validation

```{r, fig.height=4, fig.width=5}
chicken_pr = readRDS("../../Species_Objects/Cone_Objects/Chicken_clean_v3.rds")
chicken_pr = subset(chicken_pr, annotated != 'full')
chicken_pr$annotated = factor(chicken_pr$annotated, levels = c('rod','UV', 'blue', 'green', 'red', 'principle', 'accessory'))
DotPlot2(chicken_pr, features = c('CALB1', 'DSCAM', 'SOX5'), group.by = 'annotated')
VlnPlot(chicken_pr, features = c('CALB1', 'CALB2', 'DSCAM', 'SOX5', 'MYLK', 'STBD1', 'ACTN2', 'EXT1', 'GABRG3'), group.by = 'annotated', stack = TRUE, fill.by = 'ident')
```

Find DE genes the distinguish the principal from everything: 

* GABRG3 has antibodies
* EXT1 has antibodies
* LOC107055388 (MAP4K1?)

```{r, fig.width=5, fig.height=30}
de.markers = TopNDEGs(chicken_pr, group.by = 'annotated', n = 30)
DotPlot2(chicken_pr, features = unique(de.markers$gene), group.by = 'annotated')
```

Macaque calbindin

```{r, fig.height=4, fig.width=20}
macaque_initial = readRDS('../../Species_Objects/Macaque_initial.rds')
macaque_initial = UpdateEnrichment(macaque_initial, 'Macaque')
DotPlot2(subset(macaque_initial, region == 'Periphery'), features = c('CALB1', 'CALB2', 'DSCAM', 'SOX5', 'MYLK', 'STBD1', 'ACTN2'), group.by = 'annotated')
...
chicken = fullList$ChickenV3
lizard = fullList$Lizard
# chicken = readRDS("../../Species_Objects/Cone_Objects/Chicken_clean_v3.rds")
```
...
top_pr = TopNDEGs(subset(chicken, annotated %in% c('principal', 'red')), group.by = 'annotated', n = 50, avg_log2FC_cutoff = 0.25)
top_ar = TopNDEGs(subset(chicken, annotated %in% c('accessory', 'red')), group.by = 'annotated', n = 50, avg_log2FC_cutoff = 0.25)
top_pb = TopNDEGs(subset(chicken, annotated %in% c('principal', 'blue')), group.by = 'annotated', n = 50, avg_log2FC_cutoff = 0.25)
top_ab = TopNDEGs(subset(chicken, annotated %in% c('accessory', 'blue')), group.by = 'annotated', n = 50, avg_log2FC_cutoff = 0.25)
...
p2 = plot_grid(ExpressionScatter(chicken, 'red', 'principal', genes.to.label = top_pr$gene, group.by = 'annotated', label.size = 2), 
               ExpressionScatter(chicken, 'red', 'accessory', genes.to.label = top_ar$gene, group.by = 'annotated', label.size = 2))

p5 = plot_grid(ExpressionScatter(chicken, 'blue', 'principal', genes.to.label = top_pb$gene, group.by = 'annotated', label.size = 2), 
               ExpressionScatter(chicken, 'blue', 'accessory', genes.to.label = top_ab$gene, group.by = 'annotated', label.size = 2))

p2
p5# de_pr = FindMarkersFast(chicken, group.by = 'annotated', ident.1 = 'principal', ident.2 = 'red', p_val_adj_cutoff = 0.01, avg_log2FC_cutoff = 0.25)
...
```{r, fig.height=7, fig.width=14}
# top_pr = TopNDEGs(subset(lizard, annotated %in% c('principal', 'red')), group.by = 'annotated', n = 100, logFC.cutoff = 0.25)
# top_ar = TopNDEGs(subset(lizard, annotated %in% c('accessory', 'red')), group.by = 'annotated', n = 100, logFC.cutoff = 0.25)
# top_dr = merge(top_pr[top_pr$cluster == 'principal',], top_ar[top_ar$cluster == 'accessory',], by = 'gene')
# top_rd = merge(top_pr[top_pr$cluster == 'red',], top_ar[top_ar$cluster == 'red',], by = 'gene')
...
p6 = plot_grid(ExpressionScatter(lizard, 'blue', 'principal', genes.to.label = c(de_dr$gene, de_rd$gene), group.by = 'annotated', label.size = 2), 
               ExpressionScatter(lizard, 'blue', 'accessory', genes.to.label = c(de_dr$gene, de_rd$gene), group.by = 'annotated', label.size = 2))
write.xlsx(rbind(de_dr, de_rd), 'de_tests/DE_lizard_double_red_all.xlsx')
```

```{r, fig.height=12, fig.width=18}
pdf('figures/figS5_degs.pdf', height=12, width=18)
plot_grid(p1, p2, p3, p4, p5, p6, ncol = 2, align = 'v', axis = 'bt', rel_widths = c(1,2))
dev.off()
```
...
# li2ch = BlastOrthologyTable(file1 = "maps/zech/ch_to_ze.txt", 
#                             file2 = "maps/zech/ze_to_ch.txt", 
#                             keyfile1 = "Keys/Chicken_key.csv",
#                             keyfile2 = "Keys/Zebrafish_key.csv", 
#                             one2one = TRUE)
...
## Magnetoreception genes

```{r, fig.height=7, fig.width=7}
mg_genes = toupper(read.xlsx('Photoreceptor_genes_avian_photoreceptors.xlsx')$Gene.Symbol)
de_pa = FindMarkersFast(chicken, group.by = 'annotated', ident.1 = 'principal', ident.2 = 'accessory', p_val_adj_cutoff = 0.01)
ExpressionScatter(chicken, 'principal', 'accessory', genes.to.label = mg_genes[mg_genes %in% de_pa$gene], group.by = 'annotated')
write.xlsx(de_pa, 'de_tests/DE_principal_accessory_all.xlsx')

DotPlot2(chicken, group.by = 'annotated', features = c("CRY1A","CRY1B","CRY2","CRY4", "CRY1"))
DotPlot2(chicken, group.by = 'annotated', features = c('SOX5', 'DSCAM'))
```

```{r, fig.height=7, fig.width=14}
chicken_subset = subset(chicken, annotated %in% c('principal', 'accessory', 'red'))
chicken_subset$annotated2 = ifelse(chicken_subset$annotated == 'red', 'red', 'double')
de_dr = FindMarkersFast(chicken_subset, group.by = 'annotated2', ident.1 = 'double', ident.2 = 'red', p_val_adj_cutoff = 0.01)
plot_grid(ExpressionScatter(chicken_subset, 'red', 'principal', genes.to.label = mg_genes[mg_genes %in% de_dr$gene], group.by = 'annotated'), 
          ExpressionScatter(chicken_subset, 'red', 'accessory', genes.to.label = mg_genes[mg_genes %in% de_dr$gene], group.by = 'annotated'))
write.xlsx(de_dr, 'de_tests/DE_double_red_all.xlsx')
```

## Homophilic adhesion in chicken cones

```{r, fig.height=100, fig.width=10}
chicken = objectList$ChickenV3
gene2go = fread('../../reference_files/human_gene2go_symbol.csv')
cell_adhesion_terms = gene2go[grepl("cell adhesion", gene2go$GoTerm)]$GoTerm %>% unique %>% sort
cell_adhesion_terms
cell_adhesion_genes = gene2go[grepl("cell adhesion", gene2go$GoTerm)]$Symbol %>% unique %>% sort
cell_adhesion_genes

chicken = ScaleData(chicken, features = rownames(chicken))

# SeuratHeatmap(chicken, 
#               cell_adhesion_genes, 
#               group.by = 'annotated', 
...
# Lizard genes
li2ch = readRDS("orthology_graphs/li2ch.rds")
cell_adhesion_genes_converted = ConvertGeneSymbolsFromMatrix(Matrix::t(li2ch), cell_adhesion_genes) %>% 
  as.data.frame() %>% 
  rownames_to_column() %>% 
  setNames(c('Chicken', 'Lizard')) %>% 
  mutate(row_annotation = 'red')

# Filter out lowly expressed genes
objectList$Chicken$annotated = factor(objectList$Chicken$annotated)
chicken.high.features = SelectFeatures(objectList$Chicken, group.by = 'annotated', nfeatures = 100000, min.pct.expressed = 5)
objectList$Lizard$annotated = factor(objectList$Lizard$annotated)
lizard.high.features = SelectFeatures(objectList$Lizard, group.by = 'annotated', nfeatures = 100000, min.pct.expressed = 5)
cell_adhesion_genes_converted_filt = subset(cell_adhesion_genes_converted, Chicken %in% chicken.high.features & Lizard %in% lizard.high.features)

pdf('figures/cell-adhesion-molecules-filt.pdf', height = 40, width = 7)
SAMapHeatmap(objectList, 
             cell_adhesion_genes_converted_filt, 
             show_column_names = FALSE,
             species.use = c("Chicken", "Lizard"), 
             types.use = c("accessory", "principal", "red", "green", "blue", "UV", "rod"), 
             show_heatmap_legend = FALSE, 
...
connexins = sort(rownames(objectList$Chicken)[grepl('^GJA|^GJB|^GJC|^GJD|^GJE', rownames(objectList$Chicken))])
connexin_genes_converted = ConvertGeneSymbolsFromMatrix(Matrix::t(li2ch), connexins) %>% 
  as.data.frame() %>% 
  rownames_to_column() %>% 
  setNames(c('Chicken', 'Lizard')) %>% 
  mutate(row_annotation = 'red') 
  # filter(Chicken %in% chicken.high.features & Lizard %in% lizard.high.features)

SAMapHeatmap(objectList, 
             connexin_genes_converted,
             show_column_names = FALSE,
             species.use = c("Chicken", "Lizard"), 
             types.use = c("accessory", "principal", "red", "green", "blue", "UV", "rod"), 
             show_heatmap_legend = FALSE, 
...
```{r}
DotPlot2(objectList$Chicken, features = c('GJC1', 'GJD2', 'GJC2', 'GJA10', 'GJA5'), group.by = 'annotated')
DotPlot2(objectList$Lizard, features = c('GJC1', 'GJD2', 'GJC2', 'GJA10', 'GJA5'), group.by = 'annotated')
```

## Dopamine receptors for Tom

```{r, fig.height=4, fig.width=10}
# chicken = readRDS("../../Species_Objects/Cone_Objects/Chicken_clean_v3.rds")
ggarrange(
  TitlePlot(DotPlot2(chicken, features = paste0('DRD', 1:5), group.by = 'annotated'), 'Chicken'), 
  TitlePlot(DotPlot2(lizard, features = paste0('DRD', 1:5), group.by = 'annotated'), 'Lizard'), 
  ncol = 2, common.legend = TRUE, legend = 'right')

zebrafish_drd = rownames(objectList$ZebrafishAdult)[startsWith(rownames(objectList$ZebrafishAdult), 'drd')]
TitlePlot(DotPlot2(objectList$Zebrafish, features = zebrafish_drd, group.by = 'annotated'), 'Zebrafish')
DotPlot2(fullList$ZebrafishAdult, features = zebrafish_drd, group.by = 'seurat_clusters')
DimPlotLabeled(fullList$ZebrafishAdult, 'seurat_clusters') | DimPlotLabeled(fullList$ZebrafishAdult, 'annotated')
```

```{r}
neuromodulator.genes <- c("TAC1", "TACR1", "TPH1", "TPH2", "SLC6A4", "HTR1A", "HTR1B", 
           "HTR1D", "HTR1E", "HTR1F", "HTR2A", "HTR2B", "HTR2C", "HTR3A", 
           "HTR3B", "HTR3C", "HTR3D", "HTR3E", "HTR4", "HTR5A", "HTR5B", 
           "HTR6", "HTR7", "CNR1", "CNR2", "NAPEPLD", "MGLL")

ze2hs = BlastOrthologyTable(file1 = "../../../BLAST_HOMOLOGY/maps_pmNCBI_zeENS_chNCBI_liNCBI_opNCBI_sqENS_hsENS/zehs/hs_to_ze.txt",
                            file2 = "../../../BLAST_HOMOLOGY/maps_pmNCBI_zeENS_chNCBI_liNCBI_opNCBI_sqENS_hsENS/zehs/ze_to_hs.txt",
                            keyfile1 = "../../../BLAST_HOMOLOGY/Keys/Human_key.csv",
                            keyfile2 = "../../../BLAST_HOMOLOGY/Keys/Zebrafish_lyu_key.csv", 
                            one2one = TRUE)

neuromodulator.genes.ze = as.vector(ConvertGeneSymbolsFromMatrix(t(ze2hs), neuromodulator.genes))

TitlePlot(DotPlot2(fullList$ZebrafishCorbo, features = neuromodulator.genes.ze, group.by = 'annotated2') + NoLegend(), 'Corbo') | 
  TitlePlot(DotPlot2(fullList$ZebrafishDev, features = neuromodulator.genes.ze, group.by = 'annotated') + NoLegend(), 'Lyu 4/5 dpf') |
  TitlePlot(DotPlot2(fullList$ZebrafishAdult, features = neuromodulator.genes.ze, group.by = 'annotated2'), 'Lyu adult')
```
