...
#' @param remove.batches Remove integration batches smaller than this many cells, default is 60 cells
...
ClusterSeurat = function(object, nfeatures = 2000, numPCs = 20, normalization.method = "LogNormalize", scale.factor = 10000, selection.method = "vst",  
  ...
  if(!is.null(integrate.by)){
    ...
    object <- FindVariableFeatures(object, selection.method = selection.method, nfeatures = nfeatures, verbose = FALSE)
  }
...
#' Diagonalizes a set of genes for a Dot Plot
#'
...
MergeClusters <- function(object, idents, meta.data = "seurat_clusters", refactor = FALSE){
  ...
  if(refactor){
    ...
    levels(object@meta.data$seurat_clusters) = 0:(length(levels(object@meta.data$seurat_clusters))-1)
  }
