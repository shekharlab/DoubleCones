...
XGBoost_train2 = function(X_train, X_test, predict.by = 'seurat_clusters', assay = 'RNA', nround = 200, scale.mean = NULL, scale.var = NULL){
  #var.genes=NULL, do.scale=FALSE, scale.mean = NULL, scale.var = NULL, max.cells.per.ident = 400, train.frac = 0.6, 
  
  library(xgboost)
  
  # Convert to factor; if already a factor, remove unused levles
  X_train@meta.data[[predict.by]] = factor(X_train@meta.data[[predict.by]])
  
  # Get classes
  classes = levels(X_train@meta.data[[predict.by]])
  train_labels = as.numeric(X_train@meta.data[[predict.by]])-1
  test_labels = as.numeric(factor(X_test@meta.data[[predict.by]]))-1
  
  train_matrix <- xgb.DMatrix(data = t(X_train@assays[[assay]]@scale.data), label=train_labels)
  validation_matrix <- xgb.DMatrix(data = t(X_test@assays[[assay]]@scale.data), label=test_labels)
  ...
  # Train
  message('Training model on ', ncol(X_train), ' cells')
  bst_model <- xgb.train(params = xgb_params,
                         data = train_matrix,
                         nrounds = nround)
  
  # Predict hold-out validation set
  message('Testing model on ', ncol(X_test), ' cells')
  ...
  message("Test accuracy: ", signif(mean(valid_predlabels == test_labels)*100, 3), '%')
  A = table(test_labels, valid_predlabels)
  colnames(A) = classes
  rownames(A) = classes
  print(plotConfusionMatrix(A, order="Row", 
                            xlab.use = "True", 
                            ylab.use = "Predicted", 
                            plot.return = TRUE, 
...
BuildConfusionMatrix = function(test, model, test_ident = NULL, scale = TRUE, scale.by.model = FALSE, assay = "RNA", slot = "data"){
...
#' @examples
#' adult_to_larva <- BuildConfusionMatrix(larva, adult, model = larva.model)
PredictLabels = function(test, model, test_ident = NULL, scale = TRUE, scale.by.model = FALSE, assay = "RNA", slot = "data"){
  genes.use <- model$bst_model$feature_names
  train_id = factor(colnames(model$test_mat), levels = unique(colnames(model$test_mat)))
