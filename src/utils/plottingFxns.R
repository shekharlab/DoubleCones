...
plotConfusionMatrix = function(X, row.scale=TRUE, col.scale=FALSE, col.low="white", 
                               col.high="darkblue", max.size=5, ylab.use=NULL, 
                               xlab.use=NULL, order=NULL, x.lab.rot=FALSE, 
                               plot.return=TRUE, max.perc=100, stagger.threshold = 5, title = NULL){
  library(reshape2)
...
MakePrettyConfusionMatrix = function(C, xlab.use = "Training clusters", ylab.use = "Test clusters",
  ...
  row.max = apply(C,1,which.max)
  match.df = data.frame(row = names(row.max), 
                        col = row.max, 
                        match)
  ...
  p <- plotConfusionMatrix(C1[row.ord,], xlab.use = xlab.use, 
                           ylab.use = ylab.use, col.high = col.high, 
                           col.low = col.low, plot.return = TRUE)
